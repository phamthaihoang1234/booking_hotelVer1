...
public interface BookingRepository extends CrudRepository<Booking, Long> {
    ...
    @Query(value = "SELECT bookings.id,bookings.start_date,bookings.end_date,bookings.number_of_guests,bookings.room_id,bookings.num_night,bookings.total_prize,bookings.status,bookings.user_id FROM booking_hotelver15.bookings " +
    ...
    @Query(value = "SELECT bookings.id,bookings.start_date,bookings.end_date,bookings.number_of_guests,bookings.room_id,bookings.num_night,bookings.total_prize,bookings.status,bookings.user_id,rooms.name,hotel.name_of_hotel FROM booking_hotelver15.bookings " +
            "inner join rooms on bookings.room_id = rooms.id " +
...
//    boolean deleteBookingById(Long id);
    ...
    @Query(value = "SELECT bookings.id,bookings.start_date,bookings.end_date,bookings.number_of_guests,bookings.room_id,bookings.num_night,bookings.total_prize,bookings.status,bookings.user_id,rooms.name,hotel.name_of_hotel FROM booking_hotelver15.bookings " +
            "inner join rooms on bookings.room_id = rooms.id " +
            "inner join hotel on rooms.hotel_id = hotel.id and bookings.status=1 ",nativeQuery = true)
